...
public class ArrowTag extends HBox {
	...
	public ArrowTag() {
		...
		FXMLLoader fxmlLoader = fxmlLoaderFactory.create(getClass().getResource("ArrowTag.fxml"));
	...
	private void bindPosition() {
		displayManager.nodesMayHaveMovedProperty().addListener(
				(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) -> {
					repositionText();
				});
	}
	...
	private void constructString() {
		...
		if (i18nTitle != null) {
			title.setText(i18n.t(i18nTitle));
		}
		...
		for (ConditionalText conditionalText : conditionalTextElements) {
			if (conditionalText.getAppearanceCondition().getValue()) {
				...
				labelText.append(i18n.t(conditionalText.getI18nText()));
