 ...
 *
 */
//TODO: This should support filament reel changes, head changes, all of the features
public class VirtualPrinterCommandInterface extends CommandInterface {
	...
	private void handleNozzleTempChange() {
		if (nozzleHeaterModeS != HeaterMode.OFF && currentNozzleTemperatureS < nozzleTargetTemperatureS) {
		...
		else if (nozzleHeaterModeS == HeaterMode.OFF && currentNozzleTemperatureS > ROOM_TEMPERATURE) {
			currentNozzleTemperatureS -= 10;
			if (currentNozzleTemperatureS < ROOM_TEMPERATURE) {
				currentNozzleTemperatureS = ROOM_TEMPERATURE;
			}
		}
		if (nozzleHeaterModeT != HeaterMode.OFF && currentNozzleTemperatureT < nozzleTargetTemperatureT) {
		...
		else if (nozzleHeaterModeT == HeaterMode.OFF && currentNozzleTemperatureT > ROOM_TEMPERATURE) {
			currentNozzleTemperatureT -= 7;
	...
	private boolean attachReel(Filament filament, int reelNumber) throws NumberFormatException {
		...
		if (filament != null && reelNumber >= 0 && reelNumber <= 2) {
			...
			attachedReels[reelNumber] = reelFactory.create();
	...
	public RoboxRxPacket writeToPrinterImpl(RoboxTxPacket messageToWrite, boolean dontPublishResult) throws RoboxCommsException {
		...
		if (messageToWrite instanceof QueryFirmwareVersion) {
			...
			firmwareResponse.setFirmwareRevision("r" + requiredFirmwareVersionPreference.getValue().getMajor());
