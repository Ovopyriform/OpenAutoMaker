...
public class MockPrinter implements Printer {
	...
	private final ObservableList<String> gcodeTranscriptProperty = FXCollections.observableArrayList();

	private final ObjectProperty<PrinterDefinitionFile> printerDefinitionFile = new SimpleObjectProperty<>(null);

	private static Logger LOGGER = LogManager.getLogger();

	//private PrinterDefinitionFile printerConfigurationFile;

	private final MockHeadFactory testHeadFactory;
	private final ReelFactory reelFactory;

	private PrinterIdentity printerIdentity;
	private ObjectProperty<PrinterStatus> printerStatusProperty;
	private ObjectProperty<PauseStatus> pauseStatusProperty;

	private ObservableList<FirmwareError> firmwareErrorList = FXCollections.observableArrayList();
	private ObservableList<FirmwareError> activeErrorsList = FXCollections.observableArrayList();;


	@AssistedInject
	protected MockPrinter(
			MockHeadFactory testHeadFactory,
			ReelFactory reelFactory) {

		this(testHeadFactory, reelFactory, 1);
	}
	...
	public MockPrinter(
			MockHeadFactory testHeadFactory,
			ReelFactory reelFactory,
			@Assisted int numExtruders) {

		this.testHeadFactory = testHeadFactory;
	...
	private void mockPrinterIdentity() {
		printerIdentity = new PrinterIdentity();
		printerIdentity.printermodelProperty().set(RBX10);
		printerIdentity.printerFriendlyNameProperty().set("Mock " + RBX10);
		printerIdentity.printerColourProperty().set(Color.BLUE);
	}

	private void mockPrinterStatus() {
		printerStatusProperty = new SimpleObjectProperty<>(PrinterStatus.IDLE);
	}

	private void mockPauseStatus() {
		pauseStatusProperty = new SimpleObjectProperty<>(PauseStatus.NOT_PAUSED);
	}

	// For testing only allow setting of the pause status
	public void setPauseStatus(PauseStatus pauseStatus) {
		pauseStatusProperty.setValue(pauseStatus);
	}
	...
	public MockHead getHead() {
		return (MockHead) headProperty().get();
	}
	...
	public ObservableList<String> gcodeTranscriptProperty() {
		return this.gcodeTranscriptProperty;
	}

	@Override
	public ReadOnlyBooleanProperty canPauseProperty() {
		LOGGER.info("canPauseProperty not supported by MockPrinter");
		return null;
	}
	...
	public void setPrinterStatus(PrinterStatus printerStatus) {
		printerStatusProperty.set(printerStatus);
	}
	...
	public void setPrinterConfiguration(PrinterDefinitionFile printerConfigurationFile) {
		this.printerDefinitionFile.set(printerConfigurationFile);
	}
