...
public class CloseLogic {
	...
	private Optional<CloseResult> partialOpenAndCloseAtEndOfExtrusion(
		...
		double idealBValue = Math.min(1, inScopeEvents.getAvailableExtrusion()
				/ nozzleInUse.getNozzleParameters().getEjectionVolume());
	...
	protected Optional<CloseResult> overwriteClose(
		...
		if (useAvailableExtrusion
			...
			for (int movementNodeCounter = 0; movementNodeCounter < inScopeEvents.getInScopeEvents().size(); movementNodeCounter++) {
	...
	protected Optional<CloseResult> copyClose(
		...
		if (availableExtrusion >= nozzleInUse.getNozzleParameters().getEjectionVolume()) {
			...
			OUTER: for (int inScopeEventCounter = nodeToStartCopyingFromIndex; inScopeEventCounter >= 0 && inScopeEventCounter < extractedMovements.getInScopeEvents().size(); inScopeEventCounter += directionIterator) {
		...
		try {
			if (retractNode.getPriorExtrusionNode() != null) {
				...
				if (lastLayer == null) {
					...
					//                            }
					//                        }
					search: while (layerBackwardsIterator.hasNext()) {
