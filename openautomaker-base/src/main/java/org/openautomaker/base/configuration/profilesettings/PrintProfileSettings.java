...
public class PrintProfileSettings {
	...
	public PrintProfileSettings(
			I18N i18n,
			@Assisted PrintProfileSettings settingsToCopy) {
	...
	public List<Pair<PrintProfileSetting, String>> getAllEditableSettingsWithSections() {
		...
		headerSettings.forEach(setting -> {
			Pair<PrintProfileSetting, String> settingSectionPair = new Pair<>(setting, i18n.t("printProfileSettings.qualitySettings"));
			settingsAndSections.add(settingSectionPair);
		});

		tabs.forEach(tab -> {
			tab.getAllSettings().forEach(setting -> {
				Pair<PrintProfileSetting, String> settingSectionPair = new Pair<>(setting, tab.getTabName());
				settingsAndSections.add(settingSectionPair);
			});
		});
